...
```ts
let photoUrl = ""
document.querySelect(".upload").onChange = (event, file, width) => {
  event.preventDefault()
  let oFile = file[0]
  let fr = new FileReader()
  let rFilter = /(image\/png)|(image\/jepg)|(image\/jpg)|(image\/gif)/i
  if (!rFilter.test(oFile.type)) {
    console.log("请上传png，jpg或gif格式的图片")
    return
  }
  let size = Number((oFile.size / 1024 / 1024).toFixed(2))
  if (size > 10) {
    console.log("图片大小限定在10M以内")
  }
  ...
  fr.onload = function (FREvent) {
    ...
    img.onload = function () {
      ...
      dealImage(
        ...
        function (base) {
          document.querySelector("img")[0].onload = function () {
            // CanvasDemo.initImg()
            CanvasDemo.changeImg()
          }
        }
      )
    }
...
// <img src="" class="photo" id="img0" alt="">
// <canvas id="canvasId"></canvas>
const CanvasDemo = function (mw, mh) {
  let canvasEl,
    img = []
  return {
    init: function () {
      canvasEl = new Canvas.Element()
      canvasEl.init("canvasId", { width: mw, height: mh })
    },
    initImg: function () {
      let photo = new Canvas.Img("img0", {})
      photo.iconw = document.querySelector(".icon").width()
      canvasEl.addImage(photo)
    },
    getImageData: function () {
      return canvasEl._aImages[0]
    },
    changeImg: function (ids) {
      let photo = new Canvas.Img("img0", {})
      photo.iconw = document.querySelector(".icon").width()
      canvasEl._aImages[0] = photo
      canvasEl.renderTop()
    },
  }
}
...
```ts
function generateImg(url, saveObj) {
  if (!url) {
    console.log("请先上传照片")
    return
  }
  let data = CanvasDemo.getImageData()
  let canvas = document.createElement("canvas")
  canvas.width = saveObj.width
  canvas.height = saveObj.height
  let ctx = canvas.getContext("2d")
  ctx.save()
  let mbimg = new Image()
  mbimg.src = saveObj.url

  let photo = new Image()
  photo.src = photoUrl

  let rad = parseFloat((saveObj.width / mw).toFixed(2))
  let w2 = parseFloat((data.width * rad).toFixed(2))
  let h2 = parseFloat((data.height * rad).toFixed(2))
  let x = (rad * (data.left - (data.scalex * data.width) / 2)).toFixed(2)
  let y = (rad * (data.top - (data.scalex * data.height) / 2)).toFixed(2)
  console.log(w2 + "  " + h2 + "  " + x + "   " + y)
  let x = data.left * rad
  let y = data.top * rad

  photo.onload = function (ret) {
    ctx.translate(x, y)
    ctx.rotate(data.theta)
    ctx.scale(data.scalex, data.scaley)
    ctx.drawImage(photo, -w2 / 2, -h2 / 2, w2, h2)

    ctx.restore()
    ctx.drawImage(mbimg, 0, 0, saveObj.width, saveObj.height)
    // code.onload = function(){
    let code = new Image()
    code.src = "images/erweima.png"
    ctx.drawImage(code, 35, 1155, 150, 150)
    var base = canvas.toDataURL("image/png")
    document.querySelector("img").attr("src", base)
    // }
  }
}
```
...
;(function () {
  ...
  Canvas.Element.prototype._initElement = function (el) {
    if (YAHOO.util.Dom.inDocument(el)) {
      ...
      canvasEl.id = el + ""
      var oCanvas = document.body.insertBefore(
        canvasEl,
  ...
  Canvas.Element.prototype._createContainer = function () {
    ...
    var oContainer = this._oElement.parentNode.insertBefore(
      canvasEl,
      this._oElement
    )
  ...
  Canvas.Element.prototype._createCanvasBackground = function () {
    ...
    var oBackground = this._oElement.parentNode.insertBefore(
      canvasEl,
      this._oElement
    )
  ...
  Canvas.Element.prototype.setCanvasBackground = function (oImg) {
    ...
    this._oContextBackground.drawImage(
      oImg._oElement,
      0,
      0,
  ...
  Canvas.Element.prototype.onMouseUp = function (e) {
    ...
    if (
      ...
      this._currentTransform.action == "rotate"
    ) {
  ...
  Canvas.Element.prototype.onMouseDown = function (e) {
    ...
    if (!oImg) {
      this._groupSelector = { ex: mp.ex, ey: mp.ey, top: 0, left: 0 }
    } else {
  ...
  Canvas.Element.prototype.scaleImage = function (mp) {
    var lastLen = Math.sqrt(
      Math.pow(this._currentTransform.ey - this._currentTransform.top, 2) +
        Math.pow(this._currentTransform.ex - this._currentTransform.left, 2)
    )
    var curLen = Math.sqrt(
      Math.pow(mp.ey - this._currentTransform.top, 2) +
        Math.pow(mp.ex - this._currentTransform.left, 2)
    )

    this._currentTransform.target.scalex =
      this._currentTransform.scalex * (curLen / lastLen)
  ...
  Canvas.Element.prototype.rotateImage = function (mp) {
    ...
    this._currentTransform.target.theta =
      curAngle - lastAngle + this._currentTransform.theta
  }
  ...
  Canvas.Element.prototype.renderAll = function (allOnTop) {
    ...
    this._oContextTop.clearRect(
      0,
      0,
      parseInt(this._oConfig.width),
      parseInt(this._oConfig.height)
    )
    containerCanvas.clearRect(
      0,
      0,
      parseInt(this._oConfig.width),
      parseInt(this._oConfig.height)
    ...
    if (allOnTop) {
      ...
      this._oContextTop.drawImage(
        this._backgroundImg._oElement,
        0,
        0,
        originalImgSize.width,
        originalImgSize.height
    ...
    this.drawImageElement(
      this._oContextTop,
      this._aImages[this._aImages.length - 1]
    )
  }
  ...
  Canvas.Element.prototype.renderTop = function () {
    ...
    if (this._groupSelector != null) {
      ...
      this._oContextTop.fillRect(
        this._groupSelector.ex -
          (this._groupSelector.left > 0 ? 0 : -this._groupSelector.left),
        this._groupSelector.ey -
          (this._groupSelector.top > 0 ? 0 : -this._groupSelector.top),
        Math.abs(this._groupSelector.left),
        Math.abs(this._groupSelector.top)
      )
      this._oContextTop.strokeRect(
        this._groupSelector.ex -
          (this._groupSelector.left > 0
            ? 0
            : Math.abs(this._groupSelector.left)),
        this._groupSelector.ey -
          (this._groupSelector.top > 0 ? 0 : Math.abs(this._groupSelector.top)),
        Math.abs(this._groupSelector.left),
        Math.abs(this._groupSelector.top)
      )
    }
  ...
  Canvas.Element.prototype.drawImageElement = function (context, oImg) {
    ...
    var polaroidHeight =
      (oImg.height -
        originalImgSize.height -
        (oImg.width - originalImgSize.width)) /
      2
    context.drawImage(
      oImg._oElement,
  ...
  Canvas.Element.prototype.findMousePosition = function (e) {
    ...
    var safariOffsetLeft = isSafari2
      ? e.target.ownerDocument.body.offsetLeft + scrollLeft
      : 0
    var safariOffsetTop = isSafari2
      ? e.target.ownerDocument.body.offsetTop + scrollTop
      : 0
    var cx = e.clientX || e.originalEvent.touches[0].clientX
    var cy = e.clientY || e.originalEvent.touches[0].clientY
    var sx = e.screenX || e.originalEvent.touches[0].screenX
    var sy = e.screenY || e.originalEvent.touches[0].screenY
    return {
      ex: cx + scrollLeft - parentNode.offsetLeft - safariOffsetLeft,
      ey: cy + scrollTop - parentNode.offsetTop - safariOffsetTop,
      screenX: sx,
      screenY: sy,
    }
  }
   ...
   */
  Canvas.Element.prototype.drawBorder = function (
    context,
    oImg,
    offsetX,
    offsetY
    ...
    context.fillRect(
      -2 - offsetX,
      -2 - offsetY,
      oImg.width + 2 * outlinewidth,
      oImg.height + 2 * outlinewidth
    )
   ...
   * @param offsetY {Number} The vertical offset applied from the (0,0) of the canvas axis
   */
  Canvas.Element.prototype.drawCorners = function (
    context,
    oImg,
    offsetX,
    offsetY
    ...
    context.fillRect(
      oImg.width - offsetX - oImg.cornersize,
      -offsetY,
      oImg.cornersize,
      oImg.cornersize
    )
    context.fillRect(
      -offsetX,
      oImg.height - offsetY - oImg.cornersize,
      oImg.cornersize,
      oImg.cornersize
    )
    context.fillRect(
