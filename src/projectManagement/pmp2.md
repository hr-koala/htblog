...
- **项目集**：协调和统一管理一组相互关联的项目（依赖关系）
- **项目组合**：指为了实现战略目标而对一个或多个项目组合进行的集中管理（优先级排序，战略一致性，价值最大化）
- **工作绩效数据**：是项目执行的时候随时记录的，是一手资料（最原始的最基础的 不能判断好坏的）
- **工作绩效信息**：是在各个监控过程中得到，是依据实际和计划的偏差，对偏差分析，以此来对未来做出预测的（可以判断好坏的）
- **工作绩效报告**：是整个项目层面，综合的将执行和计划作比较，来确定是否需要采取行动的报告，一般适用于汇报给客户管理层看
- **事业环境因素**：不可选择的，只能适应的；系统的（如工作授权系统），组织内没有，花成本在外面买来的数据库
- **组织过程资产**：可裁剪可选择的，程序类的（如变更控制程序，风险控制程序），自己积累起来的数据库（经验教训，项目文档）
- **职能型**：一般没有全职的项目经理，是协调员角色（权力非常小，做协调员）
- **弱矩阵**：有项目经理，但权力小于职能经理（项目的预算/资源掌握职能经理处）
- **平衡矩阵**：两者权力相当，需要获取资源的时候与职能经理协商（平等协商）
- **强矩阵**：项目经理权力更高（项目资源/预算在项目经理手中）
- **启动大会**：启动阶段结束规划阶段开始前召开，还没有编写项目管理计划，目的是发布项目章程，说明项目可以开始规划了（任命项目经理，获得组织对项目的承诺）
- **开工大会**：规划结束执行开始前，已编写项目管理计划，目的是传达项目目标，阐明每个相关方的角色和职责，获得团队对项目的承诺树立团队信心。（获得团队对项目的承诺）
- **定义范围**：制定项目和产品详细描述的过程，描述产品的边界和验收标准，定义范围在收集需求之后，（使边界清晰）
- **确认范围**：正式验收已完成可交付成果的过程，作用是使验收过程具有客观性，同时通过确认每个可交付成果，简单说就是核实可交付成果，推动验收（关注对可交付成果验收）
- **控制质量**：关注成果的正确性及是否满足质量；质量控制通常先于确认范围进行，质量控制属于内部检查，由执行组织的相应质量部门实施，确认范围由外部相关方（客户/发起人）对项目可交付成果进行检查验收
- **资源平衡**：在资源需求与资源供给之间取得平衡，根据资源制约因素对开始日期和完成日期进行调整（关键路径改变，工期延期）
- **资源平滑**：对进度模型中的活动进行调整，从而使项目资源需求不超过预定的资源限制的一种技术（不能改变关键路径的活动）
- **类比估算**：以过去类似项目的参数值为基础，来估算未来项目同类参数或指标，在项目详细信息不足时，类比估算综合利用历史信息和专家判断（粗略估算，成本低，耗时少，准确度低）
- **参数估算**：利用历史数据与其他变量之间的统计关系，来估算活动参数，参数估算的准确性取决于参数模型的成熟度和基础数据的可靠性
- **三角分布**：(t0+tM+tP)/3=tE
- **贝塔分布**：(t0+4tM+tp)/6=tE (估算活动持续时间/资源/成本时，三点估算）
- **需求跟踪矩阵**：把产品需求从来源连接到能满足需求的可交付成果的一种表格，把需求和可交付成果链接在一起，或产品符合计划，需要确定需求是否完成（）
- **项目范围说明书**：对项目范围，主要可交付成果，假设条件和制约因素的描述，这里可以找到可交付成果，或具体详细文件，出现最准确估算，需要最详细的文件（和项目章程区别，高层级领导查看时去找项目章程，范围说明书一般给团队查看）范围说明书是详细的描述
  进度压缩：不缩减项目范围的前提，缩短或加快进度工期，以满足制约因素，强制日期或其他进度目标（赶工和快速跟进）
- **赶工**：通过增加成本，以最小的成本代价来压缩工期，如加班/增加资源；赶工前提条件有富余成本或资源，通过在关键路径上的活动增加资源，缩短时间（增加成本）
- **快速跟进**：把正常情况下按顺序执行的活动或阶段并行执行，关键路径串行的工作并行开展，关键路径上的活动的依赖关系非强制性的（并行，返工和风险增加）
- **成本基准**：经批准的，按时间段分配的项目预算，不包括任何管理储备，只有通过正式变更控制程序才能进行变更，用作与实际结果进行比较的依据（凡是要涉及到基准的变更一定要通过最严格正式的变更控制流程【CCB】）
- **应急储备**：在进度或成本基准内，为主动应对，已知风险而分配的时间或资金（已知-PM)
- **管理储备**：在绩效测量基准之外，留作管理控制之用的一部分项目预算或项目时间，专为项目范围内不可预见的工作而预留（未知-权限在高级管理层）
- **进度偏差**：SV = EV-PV （>0 进度提前，<0 进度落后）
- **成本偏差**：CV = EV-AC （>0 成本节约，<0 成本超支）
- **进度绩效指数**：SPI = EV/PV （>1 进度提前，<1 进度落后）
- **成本绩效指数**：CPI = EV/AC （>1 成本有结余，<1 成本超支）
- **预防成本**：预防特定项目的产品、可交付成果或服务质量低劣所带来的相关成本
- **评估成本**：评估/测量/审计和测试特定项目的产品、可交付成果或服务所带来的相关成本
- **失败成本(内部/外部)**：因产品、可交付成果或服务与相关方需求或期望不一致而导致的相关成本
- **质量成本**（一致性成本/非一致性成本）一致性成本（预防成本：打造高质量产品/评估成本：评估质量）非一致性成本（内部失败成本：项目中发现的失败/- 外部失败成本：客户发现的失败）预防成本（培训/流程文档化/设备/完成时间）评估成本（测试/破坏性试验损失/检查）内部失败成本（返工/报废）外部失败成本（债务/保修工作/失去业务）
- **六种质量工具**：流程图/因果图/直方图/亲和图/帕累托图/散点图
- **流程展缺陷**：流程图展示了引发缺陷的一系列步骤
- **鱼骨追原因**：将问题陈述的原因分解为离散的分支,有助于识别问题的主要原因或根本原因
- **直方显分布**：展示每个可交付成果的缺陷数量、缺陷成因排列、各过程的不合规次数，或项目或产品缺陷的其他变现形式
- **亲和去分类**：对潜在缺陷成因进行分类，展示最应关注的领域
- **帕累托重点**：识别造成大多数问题的少数重要原因，80%的问题由 20%的原因导致，处理那些导致缺陷最多的原因（突出重点的）
- **散点看相关**：展示两个变量之间的关系的图形，它能够展示两支轴的关系，一支轴表示过程、环境或活动的任何要素，另一轴表示缺陷质量
- **职责分配矩阵(RAM)**：显示分配给每个工作包的项目资源，用于说明工作包或活动与项目团队成员之间的关系（映射关系）
  RAM 的一个例子是 RACI(执行、负责、咨询和知情)矩阵，如果团队是由内部和外部人员组成，RACI 矩阵对明确划分角色和职责特别有用
  RACI 适用于更大更复杂的项目，如果团队成员由内部和外部共同组成，则选 RACI，否则 RAM
- **资源日历**：识别每种具体资源可用时的工作日、班次、正常营业的上下班时间、周末和假期，在规划活动期间，潜在的可用资源信息（如团队资源、设备和材料）可用于估算资源的可用性
- **项目日历**：编排开展计划活动的工作日或轮流班次，以及不开展计划活动的非工作日的日历，项目日历可以让团队很明确项目的工作日和非工作日，以及每天的工作时间和非工作时间
- **形成阶段**：本阶段团队成员相互认识，并了解项目情况及他们在项目中的正式角色与职责，这一阶段，团队成员倾向于相互独立，不开诚布公
- **震荡阶段**：本阶段团队开始从事项目工作、制定技术决策和讨论项目管理方法，团队成员不能用合作和开放的态度对待不同观点和意见，团队环境可能变得事与愿违，冲突比较剧烈
...
23、 一个项目经理被分配到一个全球项目，在规划过程中，来自不同地区的团队成员不
...
managers to ignore these compliance issues because they will be prepared when the
...
理计划，确定信息传递是否有问题，选 C。沟通管理计划规定了谁将接收什么信息，何时
分发这些信息，以及沟通的方式。
...
C：审查剩余的范围，重新确定项目的时间表，以考虑到决策的延迟。Review the
...
步应该做什么来解决这个干系人的关注？A project manager just completed the planning
...
This will result in some changes in the product definition. How should the project
...
分析评估影响，再规划应对策略并执行。选项 CE 最佳。 8.正确答案：B,C,D
