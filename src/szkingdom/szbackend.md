...
![反序列化漏洞](/images/szkingdom/szbackend-AQ6-1.png)这个例子，攻击者构造一个包含恶意命令的对象，将该对象序列化到文件，受害者程序反序列化这个文件时触发命令执行。
防范措施：
...
工具介绍：源代码检测分析管理平台 静态源代码检测分析系统，可在开发阶段准确检测和定位源代码中的安全漏洞、质量缺陷，及早完成整改闭环。
![通用安全需求](/images/szkingdom/szbackend-AQ7.png)

## 场内业务
### 回购业务
回购定义：是交易双方以债券或股票为权利质押所进行的短期资金融通业务。在质押式回购交易中，资金融入方在将债券或股票出质给资金融出方融入资金的同时，双方约定在将来某一日期由资金融入方向资金融出方返还本金和按约定回购利率计算的利息，资金融出方向资金融入方返还原出质证券。

***债券质押式回购***
债券质押回购交易指资金融入方将符合要求的债券进行质押，以相应折算率计算出的质押券价值为融资额度进行质押融资，在回购期满后返还资金同时解除债券质押，期间由登记结算机构进行质押券集中管理并作为中央对手方组织集中清算交收的交易。
**仅机构户可以参与**

***债券质押式报价回购***
质押式报价回购交易是指证券公司将自有资产作为质押物，以质押物折算后的标准券数量所对应金额作为融资的额度，通过报价方式向证券公司符合条件的客户融入资金，同时约定证券公司在回购到期时向客户返还融入资金、支付相应收益。

***交易管理***
质押权出入库： 质押券出入库、现金质押、撤单
初始交易申报： 初始交易申报、深圳不允许撤单
展期操作： 自动展期、批量展期、自动展期数量调整、展期终止操作
提前终止、到期购回： 提前终止、到期购回

***股票质押购回***
股票质押回购是指符合条件的资金融入方以所持有的股票或其他证券质押，向符合条件的资 金融出方融入资金，并约定在未来返还资金、支付息费、解除质押的交易行为。

**交易要素：**
标的证券：上交所竞价交易系统挂牌上市的A股股票、债券、基金或其  他经上交所和中国结算认可的证券。
额度： 额度包括出借人额度、质押人额度、标的券额度等
折算率： 融资额/质押证券市值
履约保障比例： 现质押资产/负债。若不考虑利息等费用，简单计算就是质押证券市值除以融资金额。关系到融      出方对合约风险的控制。
融资规模： 首次融资不低于500万，之后每次不低于50万
融出方： 证券公司、资产管理计划等

***交易管理：***
补充质押申报： 是指融入方按约定补充提交标的证券进行质押登记的交易申报
部分解除质押申报： 是指融入方解除部分标的证券或其孳息质押登记的交易申报
部分购回申报(深圳)： 是指融入偿还部分资金并解除部分质押物的交易申报
违约处置申报：按照约定需处置融入方标的证券的交易申报
违约处置撤销(深圳)：按照约定需处置融入方标的证券的交易申报
违约处置卖出： 处置卖出标的证券

***清算交收：***
T+0逐笔全额非担保交收
券商M投资者A融出200万，投资者B融出100万
如果券商备付金账户为210万，则A交收成功，则B交收失败（210-200=10不足以交收B）
如果券商备付金账户为110万，则A交收失败，则B交收成功

***约定购回***
指符合条件的客户以约定价格向其指定交易的证券公司卖出标的证券，并约定在未来某一日期客户按照另一约定价格 从证券公司购回标的证券，证券公司根据与客户签署的协议将待购回期间标的证券产生的相关孳息返还给客户的交易行为。

**清算交收**
约定购回式证券交易结算托管原则
（1）两次成交，两次结算
约定购回式证券交易采用“两次成交，两次结算”的方式，两次结算包括初始交易结算与购回交易结算。
（2）T+1全额逐笔非担保交收
约定购回式证券交易实行逐笔全额非担保交收，资金交收通过证券公司的自营专用资金交收账户和客户专用资金交收账户完成。
中国结算上海分公司在T日日终对当日达成的约定购回式证券交易（包括初始交易和购回交易）进行逐笔全额清算，计算T+1日每笔交易的证券和资金应收应付额，形成当日清算结果发送给证券公司。
 T+1日16：00，中国结算上海分公司依据T日清算结果，按成交顺序对每笔交易进行证券和资金的逐笔全额非担保交收。应付资金及应付证券足额的交收成功，否则交收失败。对单笔交易不进行部分交收处理。

***债券质押式协议回购***
指回购双方自主协商约定，由资金融入方将债券出质给资金融出方融入资金，双方约定在将来某一时期由资金融入方按协议回购利率计算的资金额向资金融出方返还资金和回购利息，同时解除债券质押登记的交易

***债券质押式三方回购***
债券质押式三方回购（三方回购）是指资金融入方（正回购方）将债券质押并以相应债券的担保品价值为融资额度向资金融出方（逆回购方）质押融资，约定未来返还资金和支付回购利息，同时解除债券质押登记的交易，并由深交所、中国结算根据办法规定提供相关的担保品管理服务的交易。

***业务比较***
|       | 债券质押式回购 |  约定购回 |  股票质押回购   |  报价回购  | 协议回购  | 
|------|  -----------  | --------- |  ------------ |  ----------- | --------|
| 支持品种 | 符合规定的债券 | 上市交易的股票、基金和债券，非流通股、限售流通股除外  | A股股票、债券、基金等，包括流通股和限售股 | 债券:债券ETF;其他证券 | 符合规定的债券 | 
| 融入方 | 机构投资者 | 符合条件客户(个人、机构、产品) | 符合条件客户(个人、机构、产品) | 证券公司 | 机构投资者 | 
| 融出方 | 符合条件客户(个人、机构、产品) | 符合条件客户(个人、机构、产品) | 符合条件客户(个人、机构、产品) | 符合条件客户(个人、机构、产品) | 机构投资者 | 
| 对手方 |  交易所撮合 | 客户与证券公司之间 | 符合条件的客户、产品户等 | 券商和投资者之间 | 机构户之间 | 
| 融资规模 | 标准券额度 | 客户额度 专户额度 | 客户额度 出借人额度 标的额度 | 产品额度 专户额度 | 无  | 
| 融资期限 | 固定期限 | 购回期限不超过182天 | 一般不超过3年  | 固定期限 | 申报自行约定 | 
| 协议签署 | 债券合格投资者质押回购权限 | 约定购回融入权限 | 股票质押融出权限 股票质押融入权限 | 报价回购资金融出权限 | 债券质押协议回购、资金融入权限、债券质押协议回购、资金融入权限 | 
| 交收方式 | 净额担保交收 | 逐笔全额非担保交收 | 逐笔全额非担保交收 | 逐笔全额非担保交收 | 逐笔全额非担保交收 | 
| 资金到账 | T+0 | T+1  | T+0 | 上海 T+0 深圳T+1  | T+0 | 

|               | 优点 | 缺点 | 
| ------------ | ----- | -----  | 
| 债券质押式回购 | 交易灵活周期短(1天期回购) | 降低债券交易效率 券商承担风险、交易主体受限(机构) | 
| 约定购回 |                      | 资金T+1到账、股份过户 | 
| 股票质押回购 | 资金T+0到账、股份质押状态、支持限售股、有一套完成违约处置流程 | 交易规模受限(初始融资规模不低于500万) | 
| 报价回购 | 券商融资，无偿还风险 | 深圳T+1交收(系统做成了T+0) | 
| 协议回购 | 无违约处置流程、风险双方承担 | 交易主体受限(机构、自营、资管) | 

***融资融券***
融资交易： 投资者向证券公司交纳一定的保证金，融入一定数量的资金买入股票的交易行为
融券交易： 投资者向证券公司交纳一定的保证金，融入一定数量的证券并卖出的交易行为。

保证金：为了控制信用风险，证券公司向投资者融资、融券前，应当事先向投资者收取一定比例的保证金。
保证金范围：保证金可以是现金，也可以以证券充抵。
保证金作用：投资者提交的保证金以及投资者融资买入的全部证券和融券卖出所得的全部价款，整体作为投资者对证券公司融资融券所生债务的担保物。

***开户资格***
1.资产与资金要求：申请开户前连续20个交易日，投资者托管在证券公司的证券市值与资金账户中的可用余额（需排除通过融资融券交易融入的证券和资金）合计不低于人民币50万元。这一要求旨在确保投资者具备一定的资金实力和风险承受能力。
2.交易经验要求：
投资者需在证券公司指定交易满6个月以上，并具备融资融券业务参与资格，或者具有在期货公司开户6个月以上并从事金融期货交易的经历。这些要求旨在验证投资者在金融市场中的实际操作经验和风险意识。
3.知识与测试要求：投资者需具备期权交易的基础知识，并通过上海证券交易所认可的相关测试。这一步骤旨在确保投资者对期权产品的特性、风险及交易规则有充分的理解和掌握。
4.模拟交易经历：投资者需具有上海证券交易所认可的期权模拟交易经历。这一要求通过模拟交易环境，帮助投资者熟悉期权交易的实际操作过程，降低真实交易中的操作风险。
5.风险承受能力评估：投资者需通过证券公司的风险承受能力评估，确保其具有与期权交易相匹配的风险识别能力和承受水平。
6.诚信记录与法规限制：投资者需具备良好的诚信记录，无严重不良记录，且不受法律、法规、规章及上海证券交易所业务规则中禁止或限制从事期权交易的情形的约束。