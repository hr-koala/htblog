...
新建一个组件，用于加载子应用 `src/components/SubContainer.vue`
...
最后在首页 `src/App.vue` 放置`<router-view />`
...
由于路由模式为 history，需要匹配子应用的入口规则，修改 `src/router/index`
...
3. 修改子应用 `vite/webpack/vue-cli` 静态资源路径的配置（详细可在文末源码查看）
